...
export default function Home() {
  ...
  return (
    <>
      ...
      <div style={{backgroundColor: state.theme === 'dark' ? '#171717':'#EBF0FF', minWidth: 375, maxWidth: 520, minHeight: 667, position: 'absolute', left: '50%', transform: 'translateX(-50%)'}}>
        ...
        <div style={{ display: 'flex' }}>
          ...
          <div style={{ paddingLeft: 20 }}>
            <Typography style={{ paddingTop: 10, fontSize: 20, fontWeight: 600, lineHeight: '30px', color: state.theme === 'dark' ? '#fff':'#0E1446' }}>{auth.fullname}</Typography>
          </div>
        </div>
        </div>
        <div>
          ...
          >
            <div className={state.theme === 'dark'?'pro_back_dark':'pro_back'}>
          ...
          <Grid container>
            <Grid item xs={6}>
              <div style={{ textAlign: 'center', padding: 30 }}>
                <Typography style={{ color: state.theme === 'dark' ? '#fff':'#0E1446', fontSize: 25, fontWeight: 600 }}>${project.fund_target.toLocaleString()}</Typography>
                <Typography style={{ color: '#8A8A8A', fontSize: 14, fontWeight: 400 }}>Target</Typography>
              </div>
            </Grid>
            <Grid item xs={6}>
              <div style={{ textAlign: 'center', padding: 30 }}>
                <Typography style={{ color: state.theme === 'dark' ? '#fff':'#0E1446', fontSize: 25, fontWeight: 600 }}>${project.fund_raised.toLocaleString()}</Typography>
        ...
        <Card sx={{ padding: '30px 24px 50px 24px', borderRadius: '30px', textAlign: 'center', borderBottomLeftRadius: 0,
    borderBottomRightRadius: 0 }} style={{backgroundColor: state.theme === 'dark' ? '#0D0D0D':'#fff'}}>
          {/* <Grid container>
            <Grid item xs={6}>
